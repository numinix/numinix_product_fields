...
// Remove old field functions file (now included in npf_functions.php)
...
if ($condition_query->Recordcount() == 0) {
    $db->Execute(
        "INSERT INTO " . TABLE_PRODUCT_TYPE_LAYOUT . " (configuration_id, configuration_title, configuration_key, configuration_value, configuration_description, product_type_id, sort_order, last_modified, date_added, use_function, set_function)
        VALUES (NULL, 'Show Condition', 'SHOW_PRODUCT_INFO_CONDITION', '0', 'Show product condition on product info page (requires Numinix Product Fields)?', 1, 17, NULL, NOW(), NULL, 'zen_cfg_select_option(array(\'1\', \'0\'),');"
    );
}
