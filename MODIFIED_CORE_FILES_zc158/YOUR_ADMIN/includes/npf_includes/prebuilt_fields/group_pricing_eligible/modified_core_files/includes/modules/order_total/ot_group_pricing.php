...
class ot_group_pricing {
  ...
  function get_order_total() {
    ...
    // modified by Numinix to exclude products that are ineligible for group pricing
    global $db;
    for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
      $products_query = "SELECT products_id  
                         FROM " . TABLE_PRODUCTS . " 
                         WHERE products_id = '" . $order->products[$i]['id'] . "'
                         AND group_pricing_eligible = 0
                         ORDER BY products_id ASC
                         LIMIT 1;";
      $products = $db->Execute($products_query);
      // check if the product is ineligible
      if ($products->RecordCount() > 0) {
        // calculate the amount to deduct from the order total
        $deduction = $order->products[$i]['final_price'] * $order->products[$i]['qty'];
        // deduct tax if tax is included in the calculation
        if ($this->include_tax != 'true') {
          $tax = zen_get_tax_rate($this->tax_class, $order->delivery['country']['id'], $order->delivery['zone_id']);
          $tax_deduction = zen_calculate_tax($deduction, $tax);
          $order_total['tax'] -= $tax_deduction;
        }
        // deduct from order total
        $order_total['totalFull'] -= $deduction;
        $order_total['total'] -= $deduction; 
      }      
    }
    // end modification
    return $order_total;
