                ...
                switch ($column_name) {
                    ...
                    case 'products_name':
                        ...
                        if (defined('TEXT_DUPLICATE_IDENTIFIER')) {
                            $name = TEXT_DUPLICATE_IDENTIFIER . " " . $name;
                            if (strlen($name) > $products_name_maxlen) {
                                $name = substr($name, 0, $products_name_maxlen - 1);
                            }
                        }
...
//            $copy_attributes_duplicates_overwrite='0';
            ...
            $copy_result = zen_copy_products_attributes($products_id_from, $products_id_to);
            if ($copy_result === true) {
                $messageStack->add_session(sprintf(TEXT_COPY_AS_DUPLICATE_ATTRIBUTES, $products_id_from, $products_id_to), 'success');
            }
        }
...
/////////////////////////////////////////////////////////////////////
        if (!empty($_POST['copy_metatags']) && $_POST['copy_metatags'] == 'copy_metatags_yes') {
            $metatags_status = $db->Execute("SELECT metatags_title_status, metatags_products_name_status, metatags_model_status, metatags_price_status, metatags_title_tagline_status
                                             FROM " . TABLE_PRODUCTS . "
                                             WHERE products_id = '" . $products_id_from . "'");
            ...
            while (!$metatags_descriptions->EOF) {//one row per language
                ...
                $messageStack->add_session(sprintf(TEXT_COPY_AS_DUPLICATE_METATAGS, (int)$metatags_descriptions->fields['language_id'], $products_id_from, $products_id_to), 'success');
        ...
        if (!empty($_POST['copy_linked_categories']) && $_POST['copy_linked_categories'] == 'copy_linked_categories_yes') {
            ...
            foreach ($categories_from as $row) {
                ...
                $db->Execute("INSERT IGNORE INTO " . TABLE_PRODUCTS_TO_CATEGORIES . "
                              ...
                              VALUES (" . $products_id_to . ", " . (int)$row['categories_id'] . ")");
                $messageStack->add_session(sprintf(TEXT_COPY_AS_DUPLICATE_CATEGORIES, (int)$row['categories_id'], $products_id_from, $products_id_to), 'success');
            }
        }

        if (!empty($_POST['copy_discounts']) && $_POST['copy_discounts'] == 'copy_discounts_yes') {
            zen_copy_discounts_to_product($products_id_from, $products_id_to);
            $messageStack->add_session(sprintf(TEXT_COPY_AS_DUPLICATE_DISCOUNTS, $products_id_from, $products_id_to), 'success');
        }

        zen_record_admin_activity('Product ' . $products_id_from . ' duplicated as product ' . $products_id_to . ' via admin console.', 'info');

        $zco_notifier->notify('NOTIFY_MODULES_COPY_TO_CONFIRM_DUPLICATE', compact('products_id', 'dup_products_id'));

        $products_id = $products_id_to;//reset for further use in price update and final redirect to new linked product or new duplicated product
    }// EOF duplication
